...
a["bin/moonc"]=function(...)rawset(_G,"arg",{...})local n=i"cc.argparse"
local s=i"cc.lfs"local h=n()
h:flag("-l --lint","Perform a lint on the file instead of compiling")h:flag("-v --version","Print version")
h:flag("-w --watch","Watch file/directory for updates")
h:option("--transform","Transform syntax tree with module")
h:mutex(h:option("-t --output-to","Specify where to place compiled files"),h:option("-o","Write output to file"),h:flag("-p","Write output to standard output"),h:flag("-T","Write parse tree instead of code (to stdout)"),h:flag("-b","Write parse and compile time instead of code(to stdout)"),h:flag("-X","Write line rewrite map instead of code (to stdout)"))
h:flag("-","Read from standard in, print to standard out (Must be only argument)")local r=_G.arg[1]=="-"
if not r then
h:argument("file/directory"):args("+")else if arg[2]~=nil then
io.stderr:write("- must be the only argument\n")error(nil,0)end end;local d=r and{}or h:parse()if d.version then local k=i"moonscript.version"
k.print_version()error(nil,0)end;function log_msg(...)
if not d.p then io.stderr:write(
table.concat({...}," ").."\n")end end
local l=i("moonscript.cmd.moonc")local u=i"moonscript.util"local c=l.normalize_dir;local m=l.compile_and_write
local f=l.path_to_target
local function w(k,q)k=c(k)q=q or{}
for j in s.dir(k)do if not j:match("^%.")then local x=k..j
if
s.attributes(x,"mode")=="directory"then w(x,q)elseif j:match("%.moon$")then table.insert(q,x)end end end;return q end
local function y(k,q)local j={}local x={}
for z,_ in ipairs(k)do local E=q and q(_)or _;if not j[E]then
table.insert(x,_)j[E]=true end end;return x end
local function p(k,q)q=q or{}
if s.attributes(k,"mode")=="directory"then for j,x in ipairs(w(k))do
table.insert(q,{x,f(x,d.output_to,k)})end else
table.insert(q,{k,f(k,d.output_to)})end;return q end
if r then local k=i"moonscript.parse"local q=i"moonscript.compile"
local j=io.stdin:read("*a")local x,z=k.string(j)if not x then error(z)end;local _,z,E=q.tree(x)if not _ then
error(q.format_error(z,E,j))end;print(_)error(nil,0)end;local v=d["file/directory"]local b={}for k,q in ipairs(v)do p(q,b)end;b=y(b,function(k)
return k[2]end)
local function g(b)
local k=i("moonscript.cmd.watchers")if k.InotifyWacher:available()then return
k.InotifyWacher(b):each_update()end;return
k.SleepWatcher(b):each_update()end
if d.watch then local k
if d.lint then local x=i"moonscript.cmd.lint"k=x.lint_file else k=m end;local q=g(b)
local j=function()local x,z=true,q()
if x then return z elseif z~="interrupted!"then error(z)end end
for x in j do local z=f(x,d.t)if d.o then z=d.o end;local _,E=k(x,z)
if d.lint then if _ then
io.stderr:write(_.."\n\n")elseif E then
io.stderr:write(x.."\n"..E.."\n\n")end elseif not _ then
io.stderr:write(table.concat({"","Error: "..x,E,"\n"},"\n"))elseif _=="build"then log_msg("Built",x,"->",z)end end;io.stderr:write("\nQuitting...\n")elseif d.lint then local k
local q=i"moonscript.cmd.lint"
for j,x in pairs(b)do local z=x[1]local _,E=q.lint_file(z)if _ then k=true
io.stderr:write(_.."\n\n")elseif E then k=true
io.stderr:write(z.."\n"..E.."\n\n")end end;if k then error(nil,0)end else
for k,q in ipairs(b)do local j,x=u.unpack(q)
if d.o then x=d.o end
local z,_=m(j,x,{print=d.p,fname=j,benchmark=d.b,show_posmap=d.X,show_parse_tree=d.T,transform_module=d.transform})if not z then io.stderr:write(j.."\t".._.."\n")
error(nil,0)end end end end;return a["bin/moonc"](...)